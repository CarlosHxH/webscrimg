...
const knowledgeBases = new Map<string, string>([
  ...
  ['gaprisa', 'https://www.gaprisa.com.br/catalogsearch/result/?q='],
]);
...
const app = new Elysia()
  ...
  .get(
    '/api/scrape/gaprisa-images',
    async ({ query }) => {
      const page = Number(query.page ?? 1);
      const limit = Number(query.limit ?? 20);

      const url = `https://www.gaprisa.com.br/catalogsearch/result/?q=${encodeURIComponent(query.query)}`;
      const { data } = await http.get(url);
      const $ = cheerio.load(data);
